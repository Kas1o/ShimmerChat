...
| 组件 | 用途 | 关键参数 |
|------|------|---------|
| `ChildListEditor` | 子节点列表编辑（添加/删除/排序/拖拽/粘贴） | `ChildNodes`, `Depth`, `OnTreeChanged`, `CopyMe` |
| `SlotEditor` | 单节点槽编辑（添加/删除/拖拽/粘贴） | `SlotNode`, `Depth`, `OnTreeChanged`, `OnSet`, `OnClear`, `CopyMe` |
| `NodePickerMenu` | 添加节点菜单（分类导航 + 搜索） | `OnSelect`, `OnClose` |
| `DropStrip` | 拖拽放置条 | `TargetList`, `InsertIndex`, `OnDropped` |
...
| `OnTreeChanged` | `EventCallback` | 是 | 树结构变更时调用 |
| `Depth` | `int` | 是 | 当前缩进层级 |
...
<SlotEditor Label="Then"
            ...
            Depth="Depth"
            OnTreeChanged="HandleChanged"
            OnSet="v => { Node.Then = v; Notify(); }"
            OnClear="() => { Node.Then = null; Notify(); }"
            CopyMe="CopyMe" />
```
