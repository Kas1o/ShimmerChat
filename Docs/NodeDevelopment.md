...
`GenerationNodeSerializer` 通过 `IPluginLoaderService.GetImplementingTypes(typeof(IGenerationNode))` 扫描所有程序集中的 `IGenerationNode` 实现。**只要实现了接口并标记了 `[NodeInfo]`，节点就会自动出现在编辑器的添加菜单中，无需额外注册。**
...
```
TreeEditor
  ├── .tn-header (标题栏: 图标 + 标签 + 名称输入框 + 操作按钮)
  ├── 如果是容器 → 展开/收起按钮
  └── .tn-body (展开时)
        ├── 有 [NodeEditor] → DynamicComponent 加载自定义编辑器
        └── 无 [NodeEditor] → NodeBodyRenderer → GenericNodeEditor<T>
              ├── [NodeProperty] 标量属性 → 表单控件
              ├── [NodeProperty] List<IGenerationNode> → 子节点列表 + DropStrip
              └── [NodeProperty] IGenerationNode → 单节点槽
```
...
<div class="my-editor">
    ...
    @* 子节点列表（基础版，无拖拽；拖拽集成见第 6 节） *@
    <div class="ge-children">
        <div class="ge-children-header">
            <span class="ge-label">Children</span>
        </div>
        @for (int i = 0; i < _childNodes.Count; i++)
        {
            <TreeEditor Node="@_childNodes[i]" Depth="@(Depth + 1)"
                        OnTreeChanged="HandleChanged" />
        }
    </div>
</div>
...
@code {
    ...
    // 以下参数可选，声明 [Parameter] 即可接收
    [Parameter] public int Depth { get; set; }
    [Parameter] public Action<IGenerationNode>? RemoveMe { get; set; }
    [Parameter] public Action<IGenerationNode>? CopyMe { get; set; }

    private IList<IGenerationNode> _childNodes => Node.Nodes;

    private async Task HandleChanged() => await OnChanged.InvokeAsync(Node);
}
```
...
| 参数名 | 类型 | 是否自动注入 | 说明 |
|--------|------|:---:|------|
| `Node` | 具体节点类型 | 是 | 类型由 `[NodeEditor(typeof(...))]` 决定 |
| `OnChanged` | `EventCallback<IGenerationNode>` | 是 | 属性变更时调用，触发树重渲染 |
| `Depth` | `int` | 否 | 当前缩进层级，声明 `[Parameter]` 即可接收 |
| `RemoveMe` | `Action<IGenerationNode>?` | 否 | 父组件移除回调 |
| `CopyMe` | `Action<IGenerationNode>?` | 否 | 复制到剪贴板回调 |
...
| `.tn-btn-*` | 按钮系列 (add, del, copy, paste, move, exp) |
...
只要声明了 `[NodeProperty]` 标记的 `List<IGenerationNode>` 属性，`GenericNodeEditor` 自动提供：
...
`[NodeProperty]` 标记的 `IGenerationNode` 属性渲染为"单节点槽"：有节点时显示 `TreeEditor`，无节点时显示添加按钮。槽也支持拖放（见下节）。
...
CascadingValue<TreeDragContext>          ← 全局级联，任何组件都能收到
  ...
  ├── GenericNodeEditor                  ← 自动编辑器，内置 DropStrip
  └── 自定义编辑器                        ← 也可使用 DropStrip（见下方集成指南）
```
...
```razor
<DropStrip TargetList="@_children" InsertIndex="0" OnDropped="HandleDrop" />
```
...
| `OnDropped` | `EventCallback` | 放置成功后回调，用于触发父组件重渲染 |

`DropStrip` 自动从 `[CascadingParameter]` 接收 `DragContext`，处理 `ondragover`/`ondrop` 并调用 `DragContext.CommitDrop()`。

**视觉**: 平时 4px 高不可见，拖过时展开至 16px 带蓝色虚线框。

### 在自定义编辑器中集成拖拽

如果你的自定义编辑器渲染子节点列表，按以下模式集成（三步）：

```razor
<div class="my-editor">
    @* 1. 列表开头放置条 *@
    <DropStrip TargetList="@_childNodes" InsertIndex="0" OnDropped="HandleChanged" />
    ...
    @for (int i = 0; i < _childNodes.Count; i++)
    {
        ...
        @* 3. 子节点之间的放置条 *@
        <DropStrip TargetList="@_childNodes" InsertIndex="@(i + 1)" OnDropped="HandleChanged" />
    }
</div>
```
...
Drop (DropStrip.OnDrop)
  → DragContext.CommitDrop(targetList, insertIndex)
