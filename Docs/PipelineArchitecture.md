...
用户发送消息
  │
  ▼
Pre-Generation Tree ──→ 构建 PreGenerationEnv（含 Transient.Fragments、Transient.Tools、Transient.API）
  │
  ▼
...
IRenderModifierNode : ITreeNode   // + Execute(RenderNodeExecutionContext)
```
...
| `/agent/{agentguid}` | — | 显示三个管线的私有树状态，链接到各自编辑页 |
